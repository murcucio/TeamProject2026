...
def display_results(papers: list[dict]) -> None:
    ...
    for index, paper in enumerate(papers, start=1):
        ...
        abstract_preview = (
            paper["abstract"][:100] + "..." if paper["abstract"] else "초록 없음"
        )
        print(f"\n[{index}] {_safe_text(paper['title'])}")
        print(f"    저자: {_safe_text(authors_preview)}")
        print(f"    연도: {_safe_text(str(paper['year']))}")
        print(f"    초록: {_safe_text(abstract_preview)}")
        print(f"    링크: {_safe_text(paper['url'])}")
        print(f"    출처: {_safe_text(paper['source'])}")
...
def demo_arxiv_search(keyword: str = "code review") -> None:
    """Demo helper for arXiv-only search output."""
